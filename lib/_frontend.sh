...
frontend_update() {
  ...
  sudo su - root <<EOF
  cd /www/wwwroot/${empresa_atualizar}
  pm2 stop ${empresa_atualizar}-frontend
  git pull
  cd /www/wwwroot/${empresa_atualizar}/frontend
  npm install
  rm -rf build
  npm run build
...
EOF
 ...
 sleep 2
  ...
  sudo su - root <<EOF
   pm2 startup
  sudo env PATH=$PATH:/usr/bin /usr/lib/node_modules/pm2/bin/pm2 startup systemd -u root --hp /www/wwwroot/
EOF
  sleep 2
}
